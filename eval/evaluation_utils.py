...
class FreiburgPCDMap():
    ...
    def get_building_map(self):
        ...
        for sample_tuple in samples_root:
            ...
            query_color = torch.tensor(query['colors'], dtype=torch.float)
            # normalize the color values /255.0
            # query_color = query_color / 255.0 NO, it is already normalized
    ...
    def evaluate_error_position(self, test_vector, coor_test):
        ...
        for vector in self.map_vectors:
            ...
            euclidean_distance = torch.cdist( test_vector, vector, p=2) # es lo mismo que F.pairwise_distance(test_vector, vector, keepdim=True)        
...
def make_collate_fn(dataset: Dataset):
    ...
    def collate_fn(data_list):
        ...
        points = data_list[0][0]['points']
        color = data_list[0][0]['colors']
...
test_collate_fn_cloudy = make_collate_fn(test_pcd_dataset_cloudy)

test_pcd_dataloader_cloudy = DataLoader(test_pcd_dataset_cloudy,
                        shuffle=False,
                        collate_fn=test_collate_fn_cloudy,
                        num_workers=0,
                        batch_size=1)
test_pcd_dataset_night = PCDDataset(test_data_night)
test_collate_fn_night = make_collate_fn(test_pcd_dataset_night)

test_pcd_dataloader_night = DataLoader(test_pcd_dataset_night,
                        shuffle=False,
                        collate_fn=test_collate_fn_night,
                        num_workers=0,
                        batch_size=1)
test_pcd_dataset_sunny = PCDDataset(test_data_sunny)
test_collate_fn_sunny = make_collate_fn(test_pcd_dataset_sunny)

test_pcd_dataloader_sunny = DataLoader(test_pcd_dataset_sunny,
                        shuffle=False,
                        collate_fn=test_collate_fn_sunny,
                        num_workers=0,
                        batch_size=1)
